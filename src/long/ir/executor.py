...
## 关于 write_file 的 content 参数（极其重要）
write_file 的 content 参数必须包含实际内容，**不能为空字符串**。
- 如果文件内容**不依赖前序步骤的结果**（如对话历史中已有数据）：直接写 content
- 如果文件内容**依赖前序步骤的结果**（如搜索后才有的数据）：**不要用 write_file**，改用 `summarize` 动作，在 args 中设置 `output_path` 参数，系统会自动将总结内容写入文件
- 正确示例：`{"action": "summarize", "args": {"output_path": "output/report.md"}}` → 系统会汇总前序步骤结果，生成报告并写入文件
- 错误示例：`{"action": "call_tool", "args": {"tool_name": "write_file", "parameters": {"path": "output/report.md", "content": ""}}}` → content 为空，执行失败
- 如果前序步骤已经获取了数据（如 query_weather），用 `summarize` + `output_path` 来生成报告，不要用 `write_file` 写空内容
